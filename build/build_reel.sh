...
#   C) crossfade A -> B, optional brand overlay, poster thumbnail, cover intro
set -euo pipefail
...
$FF -ss "$TALK_SS" -t "$TALK_DUR" -i "$TALK" -filter_complex "
  [0:v]${CROP},scale=1080:1920,unsharp=5:5:0.5:5:5:0.0,setsar=1,fps=30,format=yuv420p[v];
...
" -map "[v]" -map "[a]" -c:v libx264 -crf 19 -preset medium -pix_fmt yuv420p \
  -c:a aac -b:a 192k -movflags +faststart "$W/segA_base.mp4"

$FF -i "$W/segA_base.mp4" -ac 1 -ar 16000 "$W/talk-16k.wav"
$PY "$HERE/captions/transcribe_words.py" "$W/talk-16k.wav" "$W/talk-words.json" \
...
$FF -i "$W/segA_base.mp4" -vf "subtitles=$W/captions.ass:fontsdir=$FONTSDIR" \
  -c:v libx264 -crf 19 -preset medium -pix_fmt yuv420p -c:a copy -movflags +faststart "$W/segA.mp4"
echo "· Segment A built (clean voice, karaoke captions)"
...
# ---- Join with a gentle crossfade (talk -> card) ----
...
" -map "[v]" -map "[a]" -c:v libx264 -crf 19 -preset medium -pix_fmt yuv420p \
  -c:a aac -b:a 192k -movflags +faststart "$W/joined.mp4"

# ---- Optional brand overlay (full-frame transparent PNG) ----
SRC_V="$W/joined.mp4"
if [ -n "${LOGO:-}" ]; then
  $FF -i "$W/joined.mp4" -i "$LOGO" -filter_complex \
    "[0:v][1:v]overlay=0:0:format=auto,format=yuv420p[v]" \
    -map "[v]" -map 0:a -c:v libx264 -crf 19 -preset medium -pix_fmt yuv420p \
    -c:a copy -movflags +faststart "$W/branded.mp4"
  SRC_V="$W/branded.mp4"
fi
...
if [ "$(awk "BEGIN{print ($COVER>0)}")" = "1" ]; then
  $FF -loop 1 -t "$COVER" -i "$thumb" -i "$SRC_V" -filter_complex "
    [0:v]scale=1080:1920,setsar=1,fps=30,format=yuv420p[cov];
